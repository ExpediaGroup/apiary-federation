...
| enable_invocation_logs             | Option to enable invocation logging in log4j. By default only slow (1 minute+) invocations are logged.                                                                                                                                                                                                      |  bool  |    `false`     | no |
...
|enable_query_functions_across_all_metastores | This controls one Thrift call `get_all_functions` generally used to initialize a client and get build-in functions and registered UDFS from a Metastore. Setting this to `false` only gets functions from the `primary` metastore, this is more performant. Setting this to `true` will collate results by calling `get_all_functions` from all configured metastores, this is potentially slow if some of the metastores are slow to respond. If all metastores configured are the same version we'd also just get the same functions back so it's not very useful to call this across metastores, to keep backward compatibility set this to `true`. See also https://github.com/ExpediaGroup/waggle-dance#server |bool | false | no |
