...
## [4.8.1] - 2026-04-14
### Changed
- Updated `required_version` to support Terraform `> 0.15.0, < 1.6.0`; removed deprecated `module_variable_optional_attrs` experiment (graduated to stable in Terraform 1.3.0).
...
## [4.6.5] - 2025-09-17
### Added
- Added `wd_node_affinity` to support waggledance pod node affinity configuration.

## [4.6.4] - 2025-06-03
### Changed
- Updated splunk logging options for ecs deployment. Add extra metadata as "labels" and "tag" 
- Removed `splunk-sourcetype` field from the ecs splunk logging config
- Updated `splunk-source` feild in ecs splunk logging config to `waggledance-logs`
### Added
